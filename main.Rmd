...
ecoData <- ecoData %>%
  ...
  mutate(binomFootprint = as.factor(binomFootprint))
...
# Dataset 1 rendered from user-specifed input
mapData <- ecoData %>%
  left_join(world, by = c("country" = "region"))

# Visualization 1
# Need to create title that updates with user input
ggplot() + 
  geom_polygon(data = mapData, 
               aes(x = long, y = lat, fill = binomFootprint, group = group),
               col = "white") +
  xlab("") +
  ylab("") +
  theme_minimal()
...
ggplot(aes(population, transFootprint), data = ecoData) + 
  geom_point() + 
  facet_wrap(~ continent) + 
  xlab("population") + 
  ylab("transFootprint")

ggplot(aes(population, binomFootprint), data = ecoData) + 
  geom_point() + 
  facet_wrap(~ continent) + 
  xlab("population") + 
  ylab("binomFootprint")
