...
# Introduction
A third of all food produced for human consumption - 1.3 billions tons - is wasted every year. This amounts to an annual global loss of \$990 billion dollars, of which the United States alone contributes \$165 billion to. This is a devastating waste of resources, from the food itself to the labor, water, land, and additional assets required to produce this food. In a world where countries that starve to death still exist and global population growing exponentially, we simply can't afford to misuse our resources in this imprudent manner. 

To estimate wasted resources on a country level, we can use two existing metrics: ecological footprint and biocapacity. Ecological footprint measures the ecological assets required to sustained a given population. This is usually in the unit of hectares. This means blah blah. Biocapacity essentially measures how much nature can afford to provide. This means blah blah. A country is running an ecological deficit when its footprint exceeds its biocapacity and an ecological surplus when its biocapcity exceeds its footprint. Henceforth, we will simply refer to the ecological deficit/surplus of a nation as its net footprint. 

In this project, we explore the relationship between the net footprint and the following variables: 

* `Population` in 2016
* `GDP per capita` in 2016
* `Continent` that the country is on 
* `Total food production` in 2013
* `Greenhouse gas emission` in 2012
* `Human development index` in 2015

The data on `net ecological footprint`, `population`, `continent` and `GDP per capita` was collected by Global Footprint Network and downloaded from [Kaggle](https://www.kaggle.com/footprintnetwork/ecological-footprint). This is our main source of predictors and response variables. For this data we replaced `continent` (e.g Middle East/Central Asia) by its abbreviation (e.g AS). Originally in character format, `GDP per capita` was converted into number through very simple string manipulation (dropping the *$* sign). The next predictor on the list, `total food production`, was downloaded from [Kaggle](https://www.kaggle.com/unitednations/global-food-agriculture-statistics/data), by courtesy of the [United Nation](http://data.un.org/Explorer.aspx). This dataset spans between 1990 and 2013. We took the latest data (2013) and created a new variable by summing up food production across different categories. The `Greenhouse Gas Emission` data by Country is collected by [World Resources Institue](http://www.wri.org/resources/data-sets/cait-country-greenhouse-gas-emissions-data). Similar to the food production data, we picked the most recent year (2012) of CO2 emission for each country. Last but not least is the `Human Development Index` data in 2015, by courtesy of [UNDP](http://hdr.undp.org/en/data). 
